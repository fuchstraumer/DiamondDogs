...
layout(push_constant) uniform ubo_data {
	layout(offset = 192) vec4 parameters; // temp, freq, octaves, speed
	layout(offset = 208) uint frame;
} ubo;
...
void main() {
	...
	float temperature = ubo.parameters.x;
	float frequency = ubo.parameters.y;
	float speed = ubo.parameters.z;
	float alpha_discard_level = ubo.parameters.w;

    // Time component of vector is length of vPos, or distance from center.
    // Makes corona appear to radiate outwards.
    float t = (ubo.frame * speed) - length(vPos);
    ...
    if(fColor.a < alpha_discard_level){
      discard;
    }
