...
layout(constant_id = 4) const int samples = 16;
layout(constant_id = 5) const float Kr4PI = 0.0025f * 4.0f * 3.14159f;
layout(constant_id = 6) const float Km4PI = 0.0010f * 4.0f * 3.14159f;
...
void main(){
	float scale = 1.0f / (atmo_ubo.outerRadius - atmo_ubo.innerRadius);
	// World-space position of current vertex.
    vec3 v_pos = mat3(ubo.model) * position;
	// Eye ray.
	vec3 v_ray = v_pos - atmo_ubo.cameraPos; 
	float far_length = length(v_ray);
	v_ray /= far_length;
	// ray is now a unit vector.

	// Get closest intersection of ray with atmosphere.
	float B = 2.0f * dot(atmo_ubo.cameraPos, v_ray);
	float C = (atmo_ubo.cameraHeight * atmo_ubo.cameraHeight) - (atmo_ubo.outerRadius * atmo_ubo.outerRadius);
	float det = max(0.0f, B * B - 4.0f - C);
	float near = 0.50f * (-B - sqrt(det));

	bool in_atmo;
	atmo_ubo.cameraHeight > atmo_ubo.outerRadius ? in_atmo = false : in_atmo = true;

	vec3 start;
	if(in_atmo) {
		start = atmo_ubo.cameraPos + v_ray * near;
	} 
	else {
		start = atmo_ubo.cameraPos;
	}

	float height = length(start);
	float depth = exp(scale / scaleDepth * (atmo_ubo.innerRadius - atmo_ubo.cameraHeight));

	far_length -= near;

	float start_angle;
	if(in_atmo){
		start_angle = dot(v_ray, start) / atmo_ubo.outerRadius;
	}
	else {
	...
	float sample_length = far_length / samples;
	...
	for(int i = 0; i < samples; ++i){
		...
		float sample_depth = exp((scale / scaleDepth) * (atmo_ubo.innerRadius - height));
		float light_angle = dot(atmo_ubo.lightDir, sample_point) / height;
