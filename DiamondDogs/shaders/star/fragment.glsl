...
void main(){
    ...
    vec4 noisePos = vec4(fPos,0.01f);
    float n = noise(noisePos, 4, 0.25f, 0.70f) * 0.50f;

    // Create sunspots
    vec4 sunspotPos = vec4(fPos,1.0f) * radius;
    // Sunspot frequency
    float s = 0.30f;
    float freq = 0.01f;
    float t1 = snoise(sunspotPos * freq) - s;
    float t2 = snoise((sunspotPos + radius) * freq) - s;
    float ss = (max(t1,0.0f) * max(t2,0.0f)) * 2.0f;
    ...
    float total = n - ss;
    fragColor = (vec4(colorShift,1.0f) + color) * total;
}