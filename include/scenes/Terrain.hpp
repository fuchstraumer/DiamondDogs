...
namespace terrain_scene {
	...
	public:
		...
		TerrainScene() : BaseScene(3, 1440, 900) {

			terrain::NodeRenderer::DrawAABBs = false;
			renderSkybox = true;

			UpdateCameraPosition(glm::vec3(0.0f, 400.0f, 0.0f));
		...
		virtual void RecordCommands() override {
			...
			terrain->UpdateQuadtree(CameraPosition(), GetViewMatrix());
			...
			for (uint32_t i = 0; i < graphicsPool->size(); ++i) { 
				...
				terrain->RenderNodes(terrain_buffer, begin_info, GetViewMatrix(), CameraPosition(), viewport, scissor);
	...
	private:
		...
		void createTerrain() {
			...
			terrain->SetupNodePipeline(renderPass->vkHandle(), GetProjectionMatrix());
		}
