...
namespace star_scene {
	...
	public:
		...
		StarScene() : BaseScene(4, 1440, 900) {

			SetupRenderpass(Instance::VulpesInstanceConfig.MSAA_SampleCount);

			UpdateCameraPosition(glm::vec3(3300.0f, 0.0f, 0.0f));
		...
		virtual void RecordCommands() override {
			...
			for (uint32_t i = 0; i < graphicsPool->size(); ++i) {
				...
				if (renderStar) {
					...
					star->UpdateUBOs(GetViewMatrix(), CameraPosition());
				...
				if (renderCorona) {
					...
					corona->RecordCommands(corona_buffer, begin_info, GetViewMatrix());
	...
	private:
		...
		void createStar() {
			...
			star = std::make_unique<Star>(device.get(), 5, 3000.0f, 4000, GetProjectionMatrix());
		...
		void createSkybox() {
			...
			skybox->CreateData(transferPool.get(), descriptorPool.get(), GetProjectionMatrix());
		...
		void createCorona() {
			...
			corona->Init(renderPass->vkHandle(), GetProjectionMatrix());
