...
std::experimental::filesystem::path FindFile(const std::string& fname, const char* starting_dir, size_t depth_limit) {
    ...
    for (auto& dir_entry : stdfs::recursive_directory_iterator(file_path))
    {
        ...
        if (entry_path.extension() != file_name_path.extension())
        {
            continue;
        }
...
{
    parameters.ambient = glm::vec3(mtl.ambient[0], mtl.ambient[1], mtl.ambient[2]);
    parameters.diffuse = glm::vec3(mtl.diffuse[0], mtl.diffuse[1], mtl.diffuse[2]);
    parameters.specular = glm::vec3(mtl.specular[0], mtl.specular[1], mtl.specular[2]);
    parameters.transmittance = glm::vec3(mtl.transmittance[0], mtl.transmittance[1], mtl.transmittance[2]);
    parameters.emission = glm::vec3(mtl.emission[0], mtl.emission[1], mtl.emission[2]);
    parameters.shininess = mtl.shininess;
    parameters.ior = mtl.ior;
    parameters.dissolve = mtl.dissolve;
...
void Material::loadTexture(std::string file_path_str, const std::string& material_base_name, const char* init_dir, const texture_type type)
{
    ...
    stdfs::path file_path(file_path_str);

    if (!stdfs::exists(file_path))
    {
        file_path = FindFile(file_path.string(), init_dir, 2u);
    }

    if (!stdfs::exists(file_path))
    {
        LOG(ERROR) << "Requested file path did not exist!";
        LOG(ERROR) << "File Path: " << file_path_str;
        ...
        const std::string DEBUG_TEXTURE_PATH_STR = DEBUG_TEXTURE_PATH.string();
        loader.Load("MANGO", DEBUG_TEXTURE_PATH_STR.c_str(), (void*)this, materialTextureLoadedCallback, (void*)type_ptr);
        return;
    }

    file_path_str = stdfs::canonical(file_path).string();
