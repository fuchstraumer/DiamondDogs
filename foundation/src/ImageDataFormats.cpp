...
VkFormat ToVkFormat(const ImageFormat& format) noexcept
{
    ...
    switch (format.ComponentFormat)
    {
        case ImageComponentFormats::R8:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R8_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R8_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R8_SNORM;
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_R8_SRGB;
            return VK_FORMAT_R8_UNORM; // Default
            
        case ImageComponentFormats::R16:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R16_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R16_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R16_SNORM;
            if (dataType == ImageDataType::Float) return VK_FORMAT_R16_SFLOAT;
            return VK_FORMAT_R16_UNORM; // Default
            
        case ImageComponentFormats::R32:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R32_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R32_SINT;
            if (dataType == ImageDataType::Float) return VK_FORMAT_R32_SFLOAT;
            return VK_FORMAT_R32_SFLOAT; // Default for 32-bit is usually float
            
        case ImageComponentFormats::R64:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R64_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R64_SINT;
            return VK_FORMAT_R64_SFLOAT; // Default
            
        case ImageComponentFormats::RG8:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R8G8_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R8G8_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R8G8_SNORM;
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_R8G8_SRGB;
            return VK_FORMAT_R8G8_UNORM;
            
        case ImageComponentFormats::RG16:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R16G16_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R16G16_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R16G16_SNORM;
            if (dataType == ImageDataType::Float) return VK_FORMAT_R16G16_SFLOAT;
            return VK_FORMAT_R16G16_UNORM;
            
        case ImageComponentFormats::RG32:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R32G32_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R32G32_SINT;
            return VK_FORMAT_R32G32_SFLOAT;
            
        case ImageComponentFormats::RG64:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R64G64_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R64G64_SINT;
            return VK_FORMAT_R64G64_SFLOAT;
            
        case ImageComponentFormats::RGB8:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R8G8B8_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R8G8B8_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R8G8B8_SNORM;
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_R8G8B8_SRGB;
            return VK_FORMAT_R8G8B8_UNORM;
            
        case ImageComponentFormats::RGB16:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R16G16B16_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R16G16B16_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R16G16B16_SNORM;
            if (dataType == ImageDataType::Float) return VK_FORMAT_R16G16B16_SFLOAT;
            return VK_FORMAT_R16G16B16_UNORM;
            
        case ImageComponentFormats::RGB32:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R32G32B32_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R32G32B32_SINT;
            return VK_FORMAT_R32G32B32_SFLOAT;
            
        case ImageComponentFormats::RGB64:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R64G64B64_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R64G64B64_SINT;
            return VK_FORMAT_R64G64B64_SFLOAT;
            
        case ImageComponentFormats::RGBA8:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R8G8B8A8_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R8G8B8A8_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R8G8B8A8_SNORM;
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_R8G8B8A8_SRGB;
            return VK_FORMAT_R8G8B8A8_UNORM;
            
        case ImageComponentFormats::RGBA16:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R16G16B16A16_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R16G16B16A16_SINT;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_R16G16B16A16_SNORM;
            if (dataType == ImageDataType::Float) return VK_FORMAT_R16G16B16A16_SFLOAT;
            return VK_FORMAT_R16G16B16A16_UNORM;
            
        case ImageComponentFormats::RGBA32:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R32G32B32A32_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R32G32B32A32_SINT;
            return VK_FORMAT_R32G32B32A32_SFLOAT;
            
        case ImageComponentFormats::RGBA64:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_R64G64B64A64_UINT;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_R64G64B64A64_SINT;
            return VK_FORMAT_R64G64B64A64_SFLOAT;
            
        // Special packed formats
        case ImageComponentFormats::A2R10G10B10:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_A2R10G10B10_UINT_PACK32;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_A2R10G10B10_SINT_PACK32;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_A2R10G10B10_SNORM_PACK32;
            return VK_FORMAT_A2R10G10B10_UNORM_PACK32;
            
        case ImageComponentFormats::A2B10G10R10:
            if (dataType == ImageDataType::UInt) return VK_FORMAT_A2B10G10R10_UINT_PACK32;
            if (dataType == ImageDataType::SInt) return VK_FORMAT_A2B10G10R10_SINT_PACK32;
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_A2B10G10R10_SNORM_PACK32;
        ...
        case ImageComponentFormats::BC1:
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_BC1_RGBA_SRGB_BLOCK;
            return VK_FORMAT_BC1_RGBA_UNORM_BLOCK;
            
        case ImageComponentFormats::BC2:
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_BC2_SRGB_BLOCK;
            return VK_FORMAT_BC2_UNORM_BLOCK;
            
        case ImageComponentFormats::BC3:
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_BC3_SRGB_BLOCK;
            return VK_FORMAT_BC3_UNORM_BLOCK;
            
        case ImageComponentFormats::BC4:
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_BC4_SNORM_BLOCK;
            return VK_FORMAT_BC4_UNORM_BLOCK;
            
        case ImageComponentFormats::BC5:
            if (dataType == ImageDataType::SNorm) return VK_FORMAT_BC5_SNORM_BLOCK;
            return VK_FORMAT_BC5_UNORM_BLOCK;
            
        case ImageComponentFormats::BC6H:
            if (dataType == ImageDataType::SInt) return VK_FORMAT_BC6H_SFLOAT_BLOCK;
            return VK_FORMAT_BC6H_UFLOAT_BLOCK;
            
        case ImageComponentFormats::BC7:
            if (dataType == ImageDataType::sRGB) return VK_FORMAT_BC7_SRGB_BLOCK;
