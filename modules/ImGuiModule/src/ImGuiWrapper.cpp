...
void ImGuiWrapper::Construct(VkRenderPass renderpass)
{
    ...
    rendererContext = &RenderingContext::Get();

    if (!callbacks_registered)
    {
        RenderingContext::RegisterScrollCallback(scroll_callback_t::create<&ScrollCallback>());
        RenderingContext::RegisterCharCallback(char_callback_t::create<&CharCallback>());
        RenderingContext::RegisterMouseButtonCallback(mouse_button_callback_t::create<&MouseButtonCallback>());
        RenderingContext::RegisterKeyboardKeyCallback(keyboard_key_callback_t::create<&KeyCallback>());
    ...
    standardCursors[ImGuiMouseCursor_Arrow] = RenderingContext::CreateStandardCursor(GLFW_ARROW_CURSOR);
    standardCursors[ImGuiMouseCursor_TextInput] = RenderingContext::CreateStandardCursor(GLFW_IBEAM_CURSOR);
    standardCursors[ImGuiMouseCursor_ResizeAll] = RenderingContext::CreateStandardCursor(GLFW_ARROW_CURSOR);
    standardCursors[ImGuiMouseCursor_ResizeNS] = RenderingContext::CreateStandardCursor(GLFW_VRESIZE_CURSOR);
    standardCursors[ImGuiMouseCursor_ResizeEW] = RenderingContext::CreateStandardCursor(GLFW_HRESIZE_CURSOR);
    standardCursors[ImGuiMouseCursor_ResizeNESW] = RenderingContext::CreateStandardCursor(GLFW_ARROW_CURSOR);
    standardCursors[ImGuiMouseCursor_ResizeNWSE] = RenderingContext::CreateStandardCursor(GLFW_ARROW_CURSOR);
    standardCursors[ImGuiMouseCursor_Hand] = RenderingContext::CreateStandardCursor(GLFW_HAND_CURSOR);
...
void ImGuiWrapper::Destroy()
{
    ...
    {
        RenderingContext::DestroyCursor(cursor);
    }
...
void ImGuiWrapper::newFrame()
{
    ...
    {
        io.MouseDown[i] = mouse_pressed[i] || RenderingContext::GetMouseButton(int(i)) != 0;
    }
    ...
    if (RenderingContext::GetWindowAttribute(GLFW_FOCUSED))
    {
        const ImVec2 mouse_pos_backup = io.MousePos;
        if (io.WantSetMousePos)
        {
            RenderingContext::SetCursorPosition(static_cast<double>(mouse_pos_backup.x), static_cast<double>(mouse_pos_backup.y));
        }
        else
        {
            double mouse_x, mouse_y;
            RenderingContext::GetCursorPosition(mouse_x, mouse_y);
    ...
    if (!(io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) && !(RenderingContext::GetInputMode(GLFW_CURSOR) == GLFW_CURSOR_DISABLED))
    {
        ImGuiMouseCursor cursor = ImGui::GetMouseCursor();
        if (cursor == ImGuiMouseCursor_None || io.MouseDrawCursor)
        {
            RenderingContext::SetInputMode(GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
        }
        else
        {
            rendererContext->SetCursor(standardCursors[cursor] ? standardCursors[cursor] : standardCursors[ImGuiMouseCursor_Arrow]);
            RenderingContext::SetInputMode(GLFW_CURSOR, GLFW_CURSOR_NORMAL);
        }
    }
...
void ImGuiWrapper::updateBuffers(ImGuiFrameData* data, const size_t frame_index)
{
    ...
    if ((data->currentVboSize < vtx_size) || (data->vbo == nullptr))
    {
        ...
        for (int i = 0; i < draw_data->CmdListsCount; ++i)
        {
            ...
            copies.emplace_back(gpu_resource_data_t{ list->VtxBuffer.Data, list->VtxBuffer.Size * sizeof(ImDrawVert), 0, 0 });
        }
        ...
        const std::string vboName = std::string{ "ImGuiVBO_FrameNum" } + std::to_string(frame_index);
        data->vbo = resourceContext->CreateBuffer(&buffer_info, nullptr, copies.size(), copies.data(), resource_usage::CPU_ONLY,
            ResourceCreateMemoryStrategyMinFragmentation | ResourceCreateUserDataAsString, (void*)vboName.c_str());
    ...
    if ((data->currentEboSize < idx_size) || (data->ebo == nullptr))
    {
        ...
        {
            const ImDrawList* list = draw_data->CmdLists[i];
            copies.emplace_back(gpu_resource_data_t{ list->IdxBuffer.Data, list->IdxBuffer.Size * sizeof(ImDrawIdx), 0, 0 });
        }
        ...
        const std::string eboName = std::string{ "ImGuiIBO_FrameNum" } + std::to_string(frame_index);
        data->ebo = resourceContext->CreateBuffer(&buffer_info, nullptr, copies.size(), copies.data(), resource_usage::CPU_ONLY,
            ResourceCreateMemoryStrategyMinFragmentation | ResourceCreateUserDataAsString, (void*)eboName.c_str());
...
void ImGuiWrapper::captureMouse(GLFWwindow* window)
{
    ...
    RenderingContext::SetInputMode(GLFW_CURSOR, GLFW_CURSOR_DISABLED);
}
