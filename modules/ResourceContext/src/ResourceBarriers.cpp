...
VkPipelineStageFlags2 StageFlagsFromBufferUsageBits(const BufferUsageBits bits)
{
...
VkAccessFlags2 AccessFlagsfromBufferUsageBits(const BufferUsageBits bits)
{
...
VkPipelineStageFlags2 StageFlagsFromImageUsageBits(const ImageUsageBits bits)
{
    VkPipelineStageFlags2 stage_flags = 0;

    if ((bits & ImageUsageBits::AllVertexStage) != ImageUsageBits::Invalid)
    {
        stage_flags |= VK_PIPELINE_STAGE_2_VERTEX_INPUT_BIT;
    }
    ...
    if ((bits & ImageUsageBits::PresentSource) != ImageUsageBits::Invalid)
    {
        stage_flags |= VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT;
    }

    if (bits & ImageUsageBits::DepthOnly)
    {
        // we should not get here and have any other flags set, that'll cause bad things to happen
...
VkAccessFlags2 AccessFlagsfromImageUsageBits(const ImageUsageBits bits)
{
    VkAccessFlags2 access_flags = 0;
    ...
    {
        access_flags |= VK_ACCESS_2_SHADING_RATE_IMAGE_READ_BIT_NV;
    }
...
VkImageLayout GetLayoutFromUsageBits(const ImageUsageBits bits)
{
    if (bits & ImageUsageBits::PresentSource)
    {
...
std::optional<ImageLayouts> GetImageLayouts(const ImageUsageBits before, const ImageUsageBits after)
{
...
VkAccessFlags2 GetAccessMaskFromImageUsageBits(const ImageUsageBits bits)
{
...
VkPipelineStageFlags2 GetPipelineStageFromImageUsageBits(const ImageUsageBits bits)
{
...
void ExecuteImageBarrier(VkCommandBuffer cmd, const ImageBarrierInfo& Barrier)
{
    ...
    if (layouts)
    {
        ...
        // Determine the source and destination access masks based on the usage bits.

        vkCmdPipelineBarrier(
            cmd,
            GetPipelineStageFromImageUsageBits(Barrier.BeforeUsage),
            GetPipelineStageFromImageUsageBits(Barrier.AfterUsage),
            0,
            0, nullptr,
            0, nullptr,
            1, &imageBarrier);
