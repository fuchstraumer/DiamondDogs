...
void TerrainNode::Update(const glm::vec3& camera_position, const ViewFrustum& view) {
    const TerrainConfiguration& terrainCfg = TerrainConfiguration::Get();
	...
	const Sphere lod_sphere{ camera_position, SideLength * SwitchRatio };
	const Sphere aabb_sphere{ aabbCenter(aabb) + static_cast<float>(SideLength / 2.0f), SideLength / 2.0f };
	...
	if (LOD_Level() < MaxLOD && SphereCoincidesWithAABB(lod_sphere, aabb)) {
		if (IsLeaf()) {
			Status = NodeStatus::Subdivided;
			Subdivide();
		}
		for (auto& child : Children) {
			child->Update(camera_position, view);
		}
	}
	else if (glm::distance(camera_position, SpatialCoordinates) > terrainCfg.MaxRenderDistance) {
...
const TerrainConfiguration& TerrainConfiguration::Get() noexcept
{
    return GetMutable();
}

TerrainConfiguration& TerrainConfiguration::GetMutable() noexcept
{
    static TerrainConfiguration cfg;
    return cfg;
}
